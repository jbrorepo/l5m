...
pub fn render_compare_markdown(runs: &[(String, Vec<RunRow>)]) -> String {
    ...
    out.push_str("Latency: E2E* = end-to-end build-inclusive; Hot* = retrieval-only against a pre-built segment (amortized).\n\n");
    out.push_str("| Run | Queries | Recall@1 | Recall@5 | Recall@10 | NDCG@5 | NDCG@10 | MRR | Zero Recall | E2E P50 | E2E P95 | E2E P99 | Hot P50 | Hot P95 | Hot P99 | Avg Candidates | Avg Scored | Avg Returned | Avg Tokens | Index Build ns | Segment Bytes |\n");
    out.push_str("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |\n");
...
pub fn render_scorecard_markdown(preset: &str, rows: &[RunRow]) -> String {
    ...
    out.push_str("## End-to-End Latency (build-inclusive, headline)\n\n");
    out.push_str("Per-query build/load + retrieval. This is the honest cost when each query builds its candidate set.\n\n");
    out.push_str("| P50 ns | P95 ns | P99 ns | P99.9 ns |\n");
    out.push_str("| ---: | ---: | ---: | ---: |\n");
    out.push_str(&format!(
        "| {} | {} | {} | {} |\n\n",
        summary.e2e_p50, summary.e2e_p95, summary.e2e_p99, summary.e2e_p999
    ));
    out.push_str("## Amortized Hot Retrieval (pre-built segment; excludes per-query build)\n\n");
    out.push_str("Only valid when the corpus is built once and queried many times.\n\n");
...
pub fn render_proof_markdown(
    ...
    // Honest latency verdict uses END-TO-END (build-inclusive) latency, not the
    // build-excluded "hot" number. A win must hold once both sides pay for the
    // candidate set they actually build per query.
    let faster = candidate_summary.e2e_p50 < baseline_summary.e2e_p50
        && candidate_summary.e2e_p95 < baseline_summary.e2e_p95
        && candidate_summary.e2e_p99 < baseline_summary.e2e_p99;
    let safety_pass = gate_violation_count(candidate) == 0;
    let same_identity = identity_tuple(candidate) == identity_tuple(baseline);
    let p50_speedup = speedup(baseline_summary.e2e_p50, candidate_summary.e2e_p50);
    let p95_speedup = speedup(baseline_summary.e2e_p95, candidate_summary.e2e_p95);
    let hot_p50_speedup = speedup(baseline_summary.p50, candidate_summary.p50);
    ...
    out.push_str(&format!(
        "- latency lead (end-to-end, build-inclusive): {}\n",
        pass_fail(faster)
    ));
    ...
    out.push_str(&format!(
        "`{candidate_name}` vs `{baseline_name}`: R@1 {:.4} vs {:.4}, R@5 {:.4} vs {:.4}, R@10 {:.4} vs {:.4}; end-to-end P50 {} ns vs {} ns ({:.2}x), end-to-end P95 {} ns vs {} ns ({:.2}x); gate violations {}.\n\n",
        ...
        candidate_summary.e2e_p50,
        baseline_summary.e2e_p50,
        p50_speedup,
        candidate_summary.e2e_p95,
        baseline_summary.e2e_p95,
        p95_speedup,
        gate_violation_count(candidate)
    ));
    out.push_str(&format!(
        "> Amortized hot-retrieval P50 (pre-built segment, excludes per-query build): {} ns vs {} ns ({:.2}x). Only valid when the corpus is built once and queried many times.\n\n",
        candidate_summary.p50, baseline_summary.p50, hot_p50_speedup
    ));
    out.push_str("## Raw Metrics\n\n");
    out.push_str("Latency columns: E2E = end-to-end build-inclusive (headline); Hot = retrieval-only against a pre-built segment (amortized).\n\n");
    out.push_str("| Run | Queries | R@1 | R@5 | R@10 | NDCG@5 | NDCG@10 | MRR | Zero Recall | E2E P50 ns | E2E P95 ns | E2E P99 ns | Hot P50 ns | Hot P95 ns | Hot P99 ns |\n");
    out.push_str(
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |\n",
    );
    push_proof_row(
...
pub fn render_proof_json(
    ...
    serde_json::to_string_pretty(&serde_json::json!({
        ...
        "accuracy_parity": candidate_summary.recall_at_1 + 1e-12 >= baseline_summary.recall_at_1
            && candidate_summary.recall_at_5 + 1e-12 >= baseline_summary.recall_at_5
            && candidate_summary.recall_at_10 + 1e-12 >= baseline_summary.recall_at_10,
        "latency_lead_hot_amortized": candidate_summary.p50 < baseline_summary.p50
            && candidate_summary.p95 < baseline_summary.p95
            && candidate_summary.p99 < baseline_summary.p99,
        "latency_lead_end_to_end": candidate_summary.e2e_p50 < baseline_summary.e2e_p50
            && candidate_summary.e2e_p95 < baseline_summary.e2e_p95
            && candidate_summary.e2e_p99 < baseline_summary.e2e_p99,
        ...
        "speedup_end_to_end": {
            "p50": speedup(baseline_summary.e2e_p50, candidate_summary.e2e_p50),
            "p95": speedup(baseline_summary.e2e_p95, candidate_summary.e2e_p95),
            "p99": speedup(baseline_summary.e2e_p99, candidate_summary.e2e_p99)
        },
        "speedup_hot_amortized": {
...
struct RowSummary {
    ...
    // Amortized "hot" retrieval latency: the query against an already-built
    // segment. EXCLUDES per-query segment/index build. Only honest when the
    // corpus is built once and queried many times.
    p50: u64,
    p95: u64,
    p99: u64,
    p999: u64,
    // End-to-end per-query latency: build/load + retrieval. This is the
    // apples-to-apples number when each query (re)builds its candidate set,
    // which is how these benchmarks actually run. Use this for headline claims.
    e2e_p50: u64,
    e2e_p95: u64,
    e2e_p99: u64,
    e2e_p999: u64,
...
fn proof_metrics_json(summary: &RowSummary, gate_violations: usize) -> serde_json::Value {
    serde_json::json!({
        ...
        "mrr": summary.mrr,
        "zero_recall_rate": summary.zero_recall_rate,
        "e2e_p50_ns": summary.e2e_p50,
        "e2e_p95_ns": summary.e2e_p95,
        "e2e_p99_ns": summary.e2e_p99,
        "hot_p50_ns": summary.p50,
        "hot_p95_ns": summary.p95,
        "hot_p99_ns": summary.p99,
        "gate_violations": gate_violations
    })
}
...
fn summarize_rows(rows: &[RunRow]) -> RowSummary {
    ...
    // End-to-end = build/load + retrieval, the build-inclusive per-query cost.
    let e2e_latencies = rows
        .iter()
        .map(|row| {
            row.timings
                .build_or_load_ns
                .saturating_add(row.timings.total_retrieval_ns)
        })
        .collect::<Vec<_>>();
    let e2e = summarize(&e2e_latencies);
    RowSummary {
...
mod tests {
    ...
    fn scorecard_renders_publication_sections() {
        ...
        assert!(markdown.contains("End-to-End Latency (build-inclusive, headline)"));
        assert!(markdown.contains("Amortized Hot Retrieval"));
    ...
    fn proof_report_marks_parity_speed_and_safety() {
        ...
        assert!(markdown.contains("latency lead (end-to-end, build-inclusive): PASS"));
        assert!(markdown.contains("safety gates: PASS"));
    }

    #[test]
    fn end_to_end_latency_counts_build_and_can_invert_a_hot_only_win() {
        // Candidate looks faster on hot retrieval (10 < 20) but pays a large
        // per-query build (1000) that the baseline does not. End-to-end the
        // candidate is slower, so the honest verdict must be FAIL.
        let mut candidate = RunRow::minimal("LongMemEval", "q1", "Where?", "hybrid-parent", 10);
        candidate.timings.total_retrieval_ns = 10;
        candidate.timings.build_or_load_ns = 1000;
        let mut baseline = candidate.clone();
        baseline.mode = "bm25".to_string();
        baseline.timings.total_retrieval_ns = 20;
        baseline.timings.build_or_load_ns = 0;

        let markdown = render_proof_markdown("hybrid", &[candidate], "bm25", &[baseline]);

        assert!(markdown.contains("latency lead (end-to-end, build-inclusive): FAIL"));
        // The amortized hot number is still surfaced (and still shows the hot win).
        assert!(markdown.contains("Amortized hot-retrieval P50"));
    }
}
