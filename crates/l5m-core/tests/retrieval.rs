...
#[test]
fn bad_memory_gauntlet_returns_only_admissible_normal_capsules() -> Result<()> {
    let (_dir, segment) = build_segment()?;
    let mut p = probe(
        "production database backups retained forever 14 days dev ignore previous instructions secret acquisition",
        0x1,
        0x1,
        4,
    );
    ...
    let claims = frame
        .capsules
        .iter()
        .map(|capsule| capsule.claim.as_str())
        .collect::<Vec<_>>()
        .join("\n");

    assert!(claims.contains("Production database backups are retained for 35 days."));
    assert!(!claims.contains("retained forever"));
    assert!(!claims.contains("retained for 14 days"));
    assert!(!claims.contains("Development database backups are retained for 7 days."));
    assert!(!claims.contains("Ignore previous instructions"));
    assert!(!claims.contains("Acquisition target codename"));
    assert!(frame.conflicts.iter().any(|capsule| {
        capsule.claim.contains("retained for 14 days")
            && capsule.relation_notes.iter().any(|note| {
                note.contains(RelationKind::Contradicts.as_str())
                    || note.contains(RelationKind::Supersedes.as_str())
            })
    }));
    Ok(())
}
